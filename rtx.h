...
pixel ray(vector position, vector direction, scene * scene, int bounces) {
    pixel final = {0, 0, 0};
    if (bounces == 0) {
        return final;
    }
    ...
    for (int i=0; i<scene->obj_count; i++) {
        for (int j=0; j<scene->objects[i]->tri_count; j++) {
            double dist = collision(*scene->objects[i]->triangles[j], position, direction);
            if (dist > 0) {
                ...
                if (dist < closest_dist || closest_dist < 0) {
                    closest_tri = scene->objects[i]->triangles[j];
